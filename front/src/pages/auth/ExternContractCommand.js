...
export default class ExternContractCommand extends React.Component {
    ...
    componentDidMount() {
        ...
        if (verifSession() === false) {
            if (this.props.match.params && this.props.match.params.contract_cmd && this.props.match.params.contract_cmd !== "") {
                localStorage.setItem("contract_cmd", this.props.match.params.contract_cmd)
            ...
            let cmd = this.props.match.params.contract_cmd
            try {
                let formated_data = JSON.parse(atob(cmd))
                console.log(formated_data)
                this.setState({firstLoading: false, error: false, data: formated_data})

                if (formated_data.chain && formated_data.cmd_list && formated_data.kwargs && formated_data.contract_id) {

                    WalletService.deployContractCmd(formated_data.chain.name, formated_data.chain.type, formated_data.contract_id, {cmd_list:formated_data.cmd_list}, localStorage.getItem("usrtoken"))
                        .then(res => {
                            console.log(res)
                            if (res.status === 200 && res.succes === true) {

                                localStorage.removeItem("contract_cmd")
                    ...
                    <div className="login_form">
                        {
                            (this.state.firstLoading === true) ?
                                ...
                                <Progress active={false} percent={100} size="medium" className="custom-progress-height"
                                          color='blue'/>
                        }
                                ...
                                <div className="padding-form">
                                    ...
                                    <div align="center">
                                        ...
                                        <div style={{marginTop: 40}}>
                                            {
                                                (this.state.data.msg_list || []).map((item, key) => (
                                                    <h5 style={{marginTop: -10}}>{item}</h5>
                                                ))
                                            }
                                        </div>
